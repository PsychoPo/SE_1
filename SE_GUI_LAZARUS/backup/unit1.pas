...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Dialogs, StdCtrls, dynlibs, Windows;
...
begin
  ...
  Pointer(fArrQuickSort) := GetProcAddress(hLib, PChar(f_list[0]));
  ...
  Pointer(fArrEqu) := GetProcAddress(hLib, PChar(f_list[1]));
  if (fArrEqu <> nil) then
  begin
  ...
  SetLength(FarrC, a, a);
  Pointer(fArrPow) := GetProcAddress(hLib, PChar(f_list[2]));
  if (fArrPow <> nil) then
  begin
...
begin
  Dll_work('C++Builder_CPP.dll', func_name1);
end;
